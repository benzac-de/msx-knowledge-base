...
**On `image:context`:** [guide/images.json](../examples/guide/images.json) uses `"action": "image:context"` (show the item's own `image` full-screen) for simplicity, reusing one file for both the grid thumbnail and the full-screen view. In a real app that's usually not ideal — the thumbnail should generally be considerably smaller (per the grid-cell sizing above) than what you'd want for a full-screen view, so pointing the slideshow at a separate, larger image tends to work better in practice than `image:context`.
