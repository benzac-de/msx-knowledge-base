...
category: Reference
summary: Complete TypeScript type definitions for the MSX JSON API and TVX Plugin framework, from tvx-plugin-module.min.d.ts 0.0.79.
---

# Type Definitions

> TypeScript type definitions for TVX Plugin v0.0.79 (Module)
> Project: [https://msx.benzac.de/info/](https://msx.benzac.de/info/)
...
Base types used across all MSX and TVX interfaces.
...
All `MSX*` interfaces and types — the JSON structure definitions for building MSX pages.
...
> See: [Attached Data Examples](../experts-api/special/attached-data-examples.md)
...
### MSXAttachedApplicationMenuButton
...
### MSXAttachedApplicationSettings
...
### MSXAttachedFrameworkSettings

```typescript
/** MSX - Attached Framework Settings
 * @see: {@link https://msx.benzac.de/wiki/index.php?title=Attached_Data_Examples}
*/
declare interface MSXAttachedFrameworkSettings {
    animate: number;
    transform: number;
    input: number;
    remote: number;
...
declare interface MSXAttachedFrameworkInfo {
    ...
    version: string;
    suffix: string;
    settings: MSXAttachedFrameworkSettings;
}
```
...
### MSXAttachedDictionaryContainer
...
All `TVX*` interfaces, abstract classes, tools, and services — the plugin development API. Also includes the exported instances and instantiable classes.
...
declare interface TVXPropertyTools {
    ...
    getNum(data: any, key: string, defaultValue: number): number;
...
declare interface TVXPluginTools {
    ...
    isSameContentState(state1: MSXAttachedContentState, state2: MSXAttachedContentState): boolean;
...
> See: [Video/Audio Plugin](../experts-api/plugins/video-audio-plugin.md)

---

### TVXVideoPluginPlayer
...
## Exported Instances and Classes

The module exports the following instances (static-class equivalents) and instantiable classes:
